<template>
  <div class="userMessage-wrapper" @click="cancel">
    <transition name="fadeRight">
      <div
        v-show="this.show"
        class="userMessage-wrapper-body"
      ...
      >
        <div style="position: relative;width: 100%;height: 100%">
          <div class="top">
            ...
            <div
              style="width: 100%;height: 1px;background-color: #eee;margin-top: 10px"
            ></div>
          </div>
...
export default {
  ...
  methods: {
    ...
    loadData() {
      UserApi.getUserMessage(this.currentPage).then(
        res => {
          console.log(res);
    ...
    getTimer(timer) {
      ...
      if (this.isToday(dateTimer)) {
        ...
        return `${dateTimer.getFullYear()}-${dateTimer.getMonth() +
          1}-${dateTimer.getDay()} ${dateTimer.getHours()}:${dateTimer.getMinutes()}`;
      }
    },
    isToday(theDate) {
      let today = new Date();
