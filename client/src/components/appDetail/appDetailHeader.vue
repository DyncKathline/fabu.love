    ...
    <div class="detail-header">
      <div class="detail-header-top">
        ...
        <div class="appType-platform-wrapper">
          <div
            class="appType"
            v-show="this.appInfo.appLevel"
            v-html="getAppType()"
          ></div>
          <div class="platform">
            <i
              :class="
                this.appInfo.platform === 'ios' ? 'icon-ic_ios' : 'icon-ic_andr'
              "
            ></i
            ><span v-html="`  适用于${this.appInfo.platform}`"></span>
          </div>
        </div>
        <div class="rightwrapper" style="z-index: 1">
          <el-button class="uploadWrapper button-style-main"
            ><i class="icon-ic_upload"></i>上传新版本</el-button
          >
          <input
            title="上传应用"
            ref="referenceUpload"
            ...
            type="file"
            style="position: absolute;top: 36px;left: 0px;width: 144px;height: 48px;opacity: 0;cursor:pointer"
          />
          <button class="preview button-style-border" @click="clickPreviewBtn">
            预览
          </button>
          <button class="delete button-style-border" @click="delectApp">
            删除
          </button>

          <div
            style="width: 120px;height: 16px;background-color: #6477F2;position: absolute;top: 68px;left: 36px;border-radius: 10px;filter: blur(10px);z-index: -1"
          ></div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    delectApp(item) {
      this.$confirm("确认删除？")
        .then(_ => {
          AppResourceApi.delectApp(this.team.id, this.appInfo.id).then(
            res => {
