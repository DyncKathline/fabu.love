    ...
    <div :class="getContentClass()" v-if="this.appBaseData">
      <div :class="getLeftClass()">
        ...
        <div class="pcWrapper" v-show="!isPhone">
          ...
          <div class="info">
            <p v-if="this.appVersionInfo.versionStr" class="desc">
              版本：{{ this.appVersionInfo.versionStr }}
            </p>
            <span
              >大小：{{
                (this.appVersionInfo.size / 1024 / 1024).toFixed(1)
              }}M</span
            >
          </div>
          ...
          <el-button
            ...
            ><i
              :class="
                this.platformStr === 'ios' ? 'icon-ic_ios' : 'icon-ic_andr'
              "
            ></i>
            下载安装</el-button
          >
          ...
          reader.read().then(function processResult(result) {
            if (result.done) {
              ...
              AppResourceApi.downloadedCount(
                _this.appBaseData.id,
                _this.appVersionInfo.id
              ).then(
                () => {},
                reject => {}
              );
