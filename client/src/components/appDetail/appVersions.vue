<template>
  <div class="appVersion-wrapper">
    <div class="detail-header-bottom">
      <ul>
        <li
          v-for="(item, index) in this.headerOperationData"
          :key="index"
          class="itemwrapper"
        >
          <div class="top">
    ...
    <div v-if="this.dataArr.length > 0">
      ...
      <div class="detail-content-top">
        ...
        <div class="top-right" @click="setGrayVersion">
          <span class="icon-ic_greyfb" style="margin-right: 8px"></span
          >设置灰度版本
        </div>
      </div>
      <!--内容-->
      <el-table
        ...
        class="version-table"
      >
        <el-table-column width="60" label="" class="version-table-one">
          <template slot-scope="scope">
            <span
              :class="{
                'version-table-one-lighting':
                  appInfo.releaseVersionId &&
                  appInfo.releaseVersionId === scope.row.id
              }"
            ></span>
            <span
              :class="{
                'version-table-one-gray':
                  appInfo.grayReleaseVersionId &&
                  appInfo.grayReleaseVersionId === scope.row.id
              }"
            ></span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="下载次数" width="120">
          <template slot-scope="scope">
            <span
              style="color: #9B9B9B;display: inline-block"
              v-html="getAllowDownLoadCount(scope.row)"
            ></span>
          </template>
        </el-table-column>
        <el-table-column prop="changelog" label="更新日志"> </el-table-column>
        <el-table-column label="操作" width="200">
          <template slot-scope="scope">
            <button
              class="appversion-elButton"
              @click.stop="releaseApp(scope.row)"
            >
              <i class="icon-ic_overview"></i>
            </button>
            <button
              class="appversion-elButton"
              @click.stop="clickDownLoad(scope.row)"
            >
              <i class="icon-ic_download"></i>
            </button>
            <button
              class="appversion-elButton"
              @click.stop="clickEditor(scope.row)"
            >
              <i class="icon-ic_edit"></i>
            </button>
            <button
              class="appversion-elButton"
              @click.stop="clickDelect(scope.row)"
            >
              <i class="icon-ic_delete"></i>
            </button>
          </template>
        </el-table-column>
      </el-table>
    ...
    ></editorVersion>
    <graySetting
      v-if="this.showGraySetting"
      @cancel="cancelGraySetting"
      @graySettingSuccess="graySettingSuccess"
      :versionList="this.dataArr"
      :appInfo="this.appInfo"
    ></graySetting>
  </div>
</template>
...
export default {
  ...
  methods: {
    getAppVersionListData() {
      AppResourceApi.getAppVersionList(
        this.userteam.id,
        this.appInfo.id,
        this.currentPage
      ).then(
    ...
    clickDelect(item) {
      this.$confirm("确认删除？")
        .then(() => {
          AppResourceApi.delectAppVersion(
            this.userteam.id,
            this.appInfo.id,
            item.id
          ).then(
    ...
    getCreatTime(date) {
      ...
      return `${releaseDate.getFullYear()}-${releaseDate.getMonth() +
        1}-${releaseDate.getDate()} ${releaseDate.getHours()}:${releaseDate.getMinutes()}:${releaseDate.getSeconds()}`;
    },
    getAppSize(size) {
    ...
    getAllowDownLoadCount(item) {
      // 灰度版本
      if (
        this.appInfo.grayReleaseVersionId &&
        this.appInfo.grayReleaseVersionId === item.id
      ) {
        if (
          this.appInfo.grayStrategy &&
          this.appInfo.grayStrategy.downloadCountLimit
        ) {
    ...
    getTodayCount() {
      if (
        this.appInfo.todayDownloadCount &&
        new Date(this.appInfo.todayDownloadCount.date).toDateString() ===
