    ...
    <ul class="collectionView-wrapper-ul">
      ...
      >
        <span
          :class="
            item.platform === 'ios'
              ? 'appItem-platform-ios'
              : 'appItem-platform-android'
          "
          class="appItem-platform"
        ></span>
        <img
          class="appItem-icon"
          v-if="item.icon"
          :src="getIcon(item)"
          @click="gotoAppDetail(item)"
        />

        <div class="appItem-info">
          <div class="appItem-info-namewrapper">
            <p class="nowrap" @click="gotoAppDetail(item)">
              {{ item.appName }}
            </p>
          </div>
          <table style="width: 100%;table-layout: fixed;margin-top: 24px">
            <tr>
              ...
              <td>
                <div
                  class="appItem-info-appInfo nowrap"
                  v-html="getShortUrl(item)"
                ></div>
              </td>
            </tr>
            <tr>
              ...
              <td>
                <div class="appItem-info-appInfo nowrap">
                  {{ item.bundleId }}
                </div>
              </td>
            </tr>
        ...
        <button
          class="preview button-style-border"
          @click="clickPreviewBtn(item)"
        >
          <i class="icon-ic_overview"></i>
          <span>预览</span>
        </button>
        <button
          class="editor button-style-border"
          @click="clickEditorBtn(item)"
        >
