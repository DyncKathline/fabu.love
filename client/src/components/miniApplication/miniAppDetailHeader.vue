  ...
  <div class="miniAppDetail-wrapper">
    <div class="detail-header">
      <div class="detail-header-top">
        ...
        <button class="delete button-style-border" @click="delectMiniApp">
          删除
        </button>
      </div>
    </div>
...
export default {
  ...
  created() {
    this.$nextTick(() => {
      ...
      this.subTitleArr = [
        this.appInfo.appId,
        this.appInfo.appSecret.slice(0, 5) + "******"
      ];
  ...
  methods: {
    ...
    delectMiniApp(item) {
      this.$confirm("确认删除？")
        .then(_ => {
          MiniApi.delectApp(this.team.id, this.appInfo.id).then(
            res => {
