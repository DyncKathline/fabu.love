<template>
  <div class="miniAppDetail-wrapper">
    <!--头部-->
    <miniAppDetailHeader :appInfo="this.appInfo" v-if="this.appInfo.id">
    </miniAppDetailHeader>

    <div class="miniAppDetail-wrapper-applist-header">
      <div class="left" style="position: relative">
        <div
          style="width: 160px;height: 16px;background-color: #6477F2;position: absolute;top: 30px;left: 12px;border-radius: 10px;filter: blur(10px);z-index: -1"
        ></div>
        <el-button
          style="width: 180px"
          class="uploadWrapper button-style-main"
          @click="addMiniAppQrcode"
          ><i class="el-icon-plus"></i>添加小程序二维码</el-button
        >
      </div>
    </div>

    <qrCode
      v-if="this.downloadCodeImageArray.length > 0"
    ...
    >
      <el-form ref="form" :model="qrCodeInfo" label-width="80px">
        <el-form-item label="scene">
          <el-input
            v-model="qrCodeInfo.scene"
            placeholder="场景参数列如authcode=xxxx&match=xxxx"
          ></el-input>
        </el-form-item>
        <el-form-item label="page">
          <el-input
            v-model="qrCodeInfo.page"
            placeholder="小程序入口页面"
          ></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    getAppDetailData() {
      MiniApi.getAppDetail(this.userteam.id, this.$route.params.appId).then(
        res => {
          console.log(res);
