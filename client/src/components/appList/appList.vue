  ...
  <div class="applist-wrapper">
    ...
    <div class="applist-header">
      <div class="left" style="position: relative">
        <div
          style="width: 120px;height: 16px;background-color: #6477F2;position: absolute;top: 30px;left: 12px;border-radius: 10px;filter: blur(10px);z-index: -1"
        ></div>
        <el-button class="uploadWrapper button-style-main"
          ><i class="icon-ic_upload"></i>上传应用</el-button
        >
        <input
          alt=""
          title="上传应用"
      ...
      <div class="applist-header-right">
        <div class="platform-wrapper">
          <div
            class="platform-ios"
            :class="getActiveClass('ios')"
            @click="clickIosPlatform"
          >
            <span class="platformImg icon-ic_ios"></span>
          </div>
          <div
            class="platform-android"
            :class="getActiveClass('android')"
            @click="clickAndroidPlatform"
          >
    ...
    <!--应用列表-->
    <collectionView :dataArr="this.dataList" @gotoAppDetail="gotoAppDetail">
    </collectionView>
...
export default {
  ...
  methods: {
    loadAppList() {
      AppResourceApi.getAppList(this.currentTeam.id).then(
        response => {
          this.dataList = [];
    ...
    delectApp(item) {
      this.$confirm("确认删除？")
        .then(_ => {
          AppResourceApi.delectApp(this.currentTeam.id, item.id).then(
            res => {
              this.loadAppList();
            },
            reject => {
